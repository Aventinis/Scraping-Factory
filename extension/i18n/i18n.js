...
//
// Everything below is wrapped in an IIFE so only the single `SFI18n` name
// reaches the shared global scope — same reason as shared/logger.js's own
// IIFE (see its doc comment): popup.html loads this file and then popup.js
// as separate classic <script> tags, and popup.js declares its own
// `const { t, initI18n, ... } = ...` pulled off SFI18n. If t/initI18n/etc.
// were bare top-level declarations here instead, that second declaration
// would collide with this file's — classic scripts sharing one document
// all share one global scope — throwing "Identifier 't' has already been
// declared" and silently breaking the whole popup (this exact bug shipped
// once before this comment existed).
const SFI18n = (function () {
  ...
  // A missing key resolves to the key itself, so a gap in a dictionary
  // shows up as an obviously-wrong string in the UI instead of blank text
  // or a thrown error.
  ...
  return { SUPPORTED_LANGUAGES, FALLBACK_LANGUAGE, detectDefaultLanguage, initI18n, setLanguage, getLanguage, t };
})();

if (typeof module !== 'undefined') module.exports = SFI18n;
if (typeof self !== 'undefined') self.SFI18n = SFI18n;
