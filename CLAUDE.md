...
- „robots.txt prüfen" (`btn-check-robots` neben der URL-Anzeige) zeigt an, ob die aktuell untersuchte Seite laut robots.txt der Zielseite erlaubt oder verboten ist. Die eigentliche `robots.txt`-Abfrage läuft im Content Script (`checkRobotsTxt`/`evaluateRobotsTxt`/`parseRobotsTxt` in `content-script.js`), nicht im Popup — ein Fetch von dort ist same-origin zur untersuchten Seite und braucht deshalb keine zusätzlichen `host_permissions`, anders als ein Fetch aus dem Popup (eigener `chrome-extension://`-Origin) auf eine beliebige Fremd-Domain. Popup → Service Worker → Content Script läuft über dasselbe `CHECK_ROBOTS_TXT`-Request/Response-Muster wie `GET_LOGS`. Ausgewertet wird immer nur die generische `User-agent: *`-Gruppe (nach robots.txt-Regeln „längstes passendes Pattern gewinnt, Gleichstand zugunsten von Allow"), da die von diesem Tool generierten Skripte (Standard-`requests`-User-Agent bzw. reines Playwright/Chromium) kein eigenes Produkt-Token tragen, das eine robots.txt gezielt benennen könnte. Eine fehlende `robots.txt` (404) gilt als uneingeschränkt erlaubt; jeder andere Fehlerstatus/Netzwerkfehler wird als unbestimmtes Ergebnis angezeigt statt geraten
